...
GameWindow::game_init()
{
    ...
    image0 = al_load_bitmap("./image0.png");
    pausebackground = al_load_bitmap("./pausebackground.png");
...
GameWindow::create_tower(int type)
{
    ...
    case STORM:
        ...
        break;
    case BLACK_TOWER:
        t = new black_tower(mouse_x, mouse_y);
        break;
    default:
        break;
    }
...
{
    int state = GAME_START_SCENE;
    title = al_load_font("SHREK.ttf", 64, NULL);
    subtitle = al_load_font("SHREK.ttf", 32, NULL);
    image0 = load_bitmap_at_size("image0.png", 960, 590);
    while(state!= GAME_BEGIN)
    {
        al_draw_bitmap(image0, 0, 0, 0);
        static int lev = 1;
        if(lev == 1)
            al_draw_text(Medium_font, al_map_rgb(255,255,255), window_width/2, window_height/2-130, ALLEGRO_ALIGN_CENTER, "Level One");
        else if(lev == 2)
            al_draw_text(Medium_font, al_map_rgb(255,255,255), window_width/2, window_height/2-130, ALLEGRO_ALIGN_CENTER, "Level Two");
        else if(lev == 3)
            al_draw_text(Medium_font, al_map_rgb(255,255,255), window_width/2, window_height/2-130, ALLEGRO_ALIGN_CENTER, "Level Three");
        else if(lev == 4)
            al_draw_text(Medium_font, al_map_rgb(255,255,255), window_width/2, window_height/2-130, ALLEGRO_ALIGN_CENTER, "Level four");

        al_draw_text(title, al_map_rgb(255,255,255), SCREEN_W/2-100, SCREEN_H/2-250, ALLEGRO_ALIGN_CENTER, "TOWER GAME");
        al_draw_text(subtitle, al_map_rgb(255,165,0), 400,300, ALLEGRO_ALIGN_CENTER, "PRESS TO START");
        al_flip_display();
    ...
    if(event.type == ALLEGRO_EVENT_TIMER) {
        ...
        if(event.mouse.button == 1) {
            ...
            */
            ...
            if(mouse_hover(0, 0, 500, field_height))
            {
                menu->Change_Coin(10);
            }
        }
...
void
GameWindow::game_pause()
{
    int state = GAME_PAUSE;
    pauselist = al_load_font("pirulen.ttf",32,NULL);
    pausebackground = load_bitmap_at_size("pausebackground.png", 960, 590);
    while(state == GAME_PAUSE){
        int time = al_get_timer_count(timer);
        al_draw_bitmap(pausebackground, 0, 0, 0);
        al_draw_text(pauselist, al_map_rgb(255,255,255), SCREEN_W/2, SCREEN_H/2-100, ALLEGRO_ALIGN_CENTER, "ENTER TO RETURN");
        al_flip_display();
        al_wait_for_event(event_queue,&event);
        if (event.type == ALLEGRO_EVENT_KEY_DOWN) {
            if (event.keyboard.keycode == ALLEGRO_KEY_ENTER) {
...
void GameWindow::game_slider_scene()
{
    int state = GAME_SLIDER_SCENE;
    Slider S = Slider(280, 150);
    Slider S2 = Slider(280, 350);
    while(state == GAME_SLIDER_SCENE)
    {
        al_draw_filled_rectangle(0, 0, window_width, window_height, ORANGE_LIGHT);
        S.Draw();
        S2.Draw();
        al_flip_display();
        ...
        if(event.type == ALLEGRO_EVENT_MOUSE_AXES)
        {
            ...
            if(S2.isDragged())
            {
                ...
                set_attack_volume(S2.getDegree());
            }
        }
